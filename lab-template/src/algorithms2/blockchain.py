...
class BlockchainNode(Blockchain):
    ...
    def create_transaction(self):
        ...
        signed_tx = SignedTransaction(
            tx, signature, self.crypto.key_to_bin(self.my_peer.key.pub())
        )
    ...
    async def on_transaction(self, peer: Peer, payload: SignedTransaction) -> None:
        ...
        try:
            valid_signature = self.crypto.is_valid_signature(
                self.crypto.key_from_public_bin(payload.public_key),
        ...
        print(
            f"Valid transaction {payload.transaction.nonce} from {payload.transaction.sender}"
        )
        # Add to pending transactions
        if (tx.sender, tx.nonce) not in [
            (t.sender, t.nonce) for t in self.finalized_txs
        ] and (tx.sender, tx.nonce) not in [
            (t.sender, t.nonce) for t in self.pending_txs
        ]:

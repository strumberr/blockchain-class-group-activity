...
from communication.blocks import BlockMessage, Blockchain
...
from communities.node_types import MINER_NODE


class Miner(Community, ABC):
    def started(self) -> None:
        ...
        self.blockchain = Blockchain(self.node_id, self.difficulty_target)
    ...
    def verify_signature(self, payload, tx: Transaction):
        # Verify the signature of the transaction
        print(f"Verifying signature for transaction {tx.nonce} from {tx.sender}")
        try:
            valid_signature = self.crypto.is_valid_signature(
                self.crypto.key_from_public_bin(b64decode(payload.public_key)),
                crypto_utils.serialize_dict(tx.to_dict()),
                payload.signature,
            )
        ...
        print(
            bcolors.GREEN
            + f"Valid transaction {tx.nonce} from {tx.sender}"
            + bcolors.RESET
        )
        return True
    ...
    async def on_transaction(self, peer: Peer, payload: SignedTransaction) -> None:
        ...
        if self.generate_tx_id(tx) in self.saved_txs_hashes:
            print(
                bcolors.WARNING
                + f"Transaction {tx.nonce} already received"
                + bcolors.RESET
            )
            return
    ...
    def stop_mining(self):
        self.active_mining = (
            False  # Set the flag to false to indicate that mining should stop
        )

    def remove_transactions_from_mempool(self, block):
        transactions = json.loads(block.transaction_tx)
        tx_ids_to_remove = {
            self.generate_tx_id(Transaction(**tx_data)) for tx_data in transactions
        }
        removed_txs = [
            tx for tx in self.mempool if self.generate_tx_id(tx) in tx_ids_to_remove
        ]
        self.mempool = [
            tx for tx in self.mempool if self.generate_tx_id(tx) not in tx_ids_to_remove
        ]
    ...
    async def on_block_message(self, peer: Peer, payload: BlockMessage) -> None:
        ...
        if not block.block_hash.startswith("0" * self.difficulty_target):
            ...
            print(bcolors.OK + f"Valid proof of work for block {block.block_hash}")
        ...
        if self.create_merkle_root(transactions_received) != block.merkle_root:
            ...
            print(bcolors.OK + f"Valid merkle root for block {block.block_hash}")
        ...
        if block.block_hash != self.compute_hash(block, block.nonce):
            ...
            print(bcolors.OK + f"Valid block hash for block {block.block_hash}")
